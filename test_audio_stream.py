# file: test_audio_stream_crossfade.py  (overwrite previous)
import time
...
def preprocess_clip(path: Path) -> Path:
    stats = loudnorm_stats(path)
    conformed = conform_clip(
        path,
        float(stats["input_i"]),
        float(stats["input_tp"]),
        tmp_dir=TMP_DIR
    )
    return conformed
...
def load_clean(path: Path) -> AudioSegment:
    ...
    seg = strip_silence(seg,
                        silence_thresh=TRIM_THRESHOLD_DB,
                        padding=PADDING_MS)
...
def main():
    ...
    print(f"▶️ Playing {clips[0].name} → cross-fading into {clips[1].name} over "
          f"{CROSSFADE_MS} ms at the very end of clip-1")
